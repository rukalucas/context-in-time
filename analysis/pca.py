...
def parse_projection(projection_spec, model):
    ...
    if isinstance(projection_spec, str):
        ...
        if len(parts) != 3:
            raise ValueError(
                f"Invalid projection spec: {projection_spec}. Expected format: 'output_dim_0' or 'input_dim_2'"
            )
        ...
        if direction_type == "output":
            ...
            raise ValueError(
                f"Unknown direction type: {direction_type}. Must be 'output' or 'input'"
            )
    ...
    raise TypeError(
        f"projection_spec must be str or np.ndarray, got {type(projection_spec)}"
    )
...
def _extract_periods(hidden_np, period_boundaries_list, period_list):
    ...
    for i in range(num_trials):
        ...
        for period_name in period_list:
            ...
            extracted_data[i, idx : idx + period_len, :] = hidden_np[
                trial_idx, :, start:end
            ].T
...
def do_pca(
    ...
    if projection is not None:
        ...
        for proj_vec in projection_vectors:
            ...
            flat_proj = np.concatenate(
                [proj_onto[i, : lengths[i]] for i in range(num_trials)]
            )
        ...
        axis_labels = [
            f"PC{i + 1} ({explained_variance[i]:.1%} var)" for i in range(n_components)
        ]
...
def plot_cross_period_variance(
    data_dict, task, period_names=None, n_components=3, figsize=(8, 7), save_path=None
):
    """
    ...
    for i in range(n_periods):
        for j in range(n_periods):
            ax.text(
                j,
                i,
                f"{variance_matrix[i, j]:.1f}",
                ha="center",
                va="center",
                color="white" if variance_matrix[i, j] < 50 else "black",
                fontsize=11,
            )
    ...
    ax.set_title(
        f"Cross-Period Variance Explained by Top {n_components} PCs (%)",
        fontsize=13,
        pad=10,
    )
...
def visualize_pca(
    ...
    for i in range(num_trials):
        ...
        if segments is not None and events is not None:
            ...
            for seg_spec in segments:
                ...
                if start_idx is None or end_idx is None:
                    ...
                    seg_label = seg_spec.get(
                        "label", f"{seg_spec['start']}→{seg_spec['end']}"
                    )
                    if i == 0:  # Only warn on first trial
                        print(
                            f"Warning: Segment '{seg_label}' (events {seg_spec['start']}→{seg_spec['end']}) "
        ...
        if plot_3d:
            start_points.append(
                [pca_data[i, 0, 0], pca_data[i, 0, 1], pca_data[i, 0, 2]]
            )
            end_points.append(
                [
                    pca_data[i, T_trial - 1, 0],
                    pca_data[i, T_trial - 1, 1],
                    pca_data[i, T_trial - 1, 2],
                ]
            )
        else:
            start_points.append([pca_data[i, 0, 0], pca_data[i, 0, 1]])
            end_points.append(
                [pca_data[i, T_trial - 1, 0], pca_data[i, T_trial - 1, 1]]
            )
    ...
    if plot_3d:
        for segment, color, alpha in zip(segments_3d, colors_3d, alphas_3d):
            line_collection = Line3DCollection(
                [segment], colors=[color], linewidths=1.5, alpha=alpha
            )
...
def animate_pca(
    ...
    title = (
        f"RNN Trajectories ({'3D' if plot_3d else '2D'}) - All Trials Evolving Together"
        if plot_simultaneously
        else f"RNN Trajectories ({'3D' if plot_3d else '2D'}) - Sequential"
    )
    if period_name:
    ...
    time_text = ax_time.text(
        0.5, 0.5, "", ha="center", va="center", fontsize=16, fontweight="bold"
    )
        ...
        for val, label in labels.items():
            if val in color_map:
                legend_patches.append(
                    mpatches.Patch(color=color_map[val], label=label, alpha=0.7)
                )
        if legend_patches:
    ...
    for i in range(num_trials):
        ...
        if plot_3d:
            ...
            end_marker = ax_main.scatter(
                [pca_data[i, period_len - 1, 0]],
    ...
    def animate(frame):
        ...
        for i in range(num_trials):
            ...
            if frame < period_len:
                ...
                lines[i].set_data(
                    pca_data[i, : frame + 1, 0], pca_data[i, : frame + 1, 1]
                )
                ...
                if hasattr(lines[i], "set_3d_properties"):
                    ...
                    current_points[i].set_data(
                        [pca_data[i, frame, 0]], [pca_data[i, frame, 1]]
                    )
            elif frame >= period_len:
                # Trial has ended
                lines[i].set_data(
                    pca_data[i, :period_len, 0], pca_data[i, :period_len, 1]
                )
        ...
        if period_name:
            time_text.set_text(
                f"{period_name.upper()}: {current_time_ms} ms from period start"
            )
        else:
